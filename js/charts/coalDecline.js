 ...
 * 
 * Shows coal consumption over time - tracking the decline in developed nations.
 */
...
function prepareData(rawData, filterState) {
  ...
  if (filterState.country === 'All Countries') {
    return aggregateByYear(inRange, 'coal_consumption');
  } else {
    return inRange
      .map(d => ({ year: d.year, value: d.coal_consumption || 0 }))
      .sort((a, b) => a.year - b.year);
  }
}

export function renderCoalDecline(container, data, filterState) {
  ...
  // Create vibrant gradient for coal
  const gradient = svg.append('defs')
    .append('linearGradient')
    .attr('id', 'coal-gradient')
    .attr('x1', '0%')
    .attr('x2', '0%')
    .attr('y1', '0%')
    .attr('y2', '100%');
  ...
  gradient.append('stop')
    .attr('offset', '0%')
    .attr('stop-color', '#dc2626') // Red at top
    .attr('stop-opacity', 0.8);
  
  gradient.append('stop')
    .attr('offset', '50%')
    .attr('stop-color', '#f97316') // Orange middle
    .attr('stop-opacity', 0.6);
  
  gradient.append('stop')
    .attr('offset', '100%')
    .attr('stop-color', '#eab308') // Yellow at bottom
  ...
  function render(isInitial = false) {
    ...
    circles.enter().append('circle')
      ...
      .on('mouseover', function(event, d) {
        ...
        tooltip.show(`<strong>Year:</strong> ${d.year}<br/><strong>Coal:</strong> ${formatNumber(d.value)} TWh`, event);
      })
...
export default renderCoalDecline;
